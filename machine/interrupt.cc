...
// advance simulated time
    ...
    DEBUG('i', "\n== Tick %d ==\n", stats->totalTicks);
 //时间片轮转调度时取消注释if(currentThread->getThreadID() != 0)
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/

    //printf("*** thread %d,  with %d time slices left, priority = %d\n", currentThread->getThreadID(), currentThread->getTimeSlice(), currentThread->getPriority());

    if(currentThread->getTimeSlice() <= 0)//     running out of time slice, re-arrange
    {
        //printf("thread %d running out of time slices\n\n",
               //currentThread->getThreadID());

        currentThread->setTimeSlice(TIME_SLICE);
        currentThread->Yield();
        //int priority = currentThread->getPriority();
        //currentThread->MultiQueueYield(priority + 1);
    }
    else
        currentThread->timeSliceMinus(10);
